...
- When a second pilot (e.g. the judge on the phone controller) starts
  ...
  returns to yours.
- A **blue 3D tracker arrow** floats above your suit and points at that pilot
  (the nearest one if there are several), with their name, distance and
  height difference (`▲` above you, `▼` below) beside it. Ahead reads up the
  screen and behind reads down it, like a satnav arrow. It fades out once
  they're within ~15 m, and while you watch their suit (`V`) it points back
  at yours.
...
## Worlds & Environmental Intelligence

The client hosts two worlds, chosen with `?world=<id>` (or the WORLD dropdown,
which reloads the page):

| id | What it is |
| --- | --- |
| `jhu` (default) | Homewood training/combat world — unchanged |
| `california-wildfire` | Sierra Nevada, real NASA FIRMS active-fire detections, evidence panel, observation priority, human-accepted inspection missions |

The wildfire world ships with a real, unmodified, clearly labelled
`HISTORICAL DEMO` FIRMS dataset and can use live FIRMS in local development
through a Vite dev proxy that keeps the `FIRMS_MAP_KEY` server-side. Full
architecture, data flow, test procedure and limitations:
[`docs/environmental-intelligence.md`](docs/environmental-intelligence.md).
...
# VITE_FIRMS_MODE=demo         # 'demo' (bundled historical) | 'live' (dev proxy, falls back to demo)
# VITE_FIRMS_PROXY_URL=/api/firms
# VITE_FIRMS_DAYS=2            # live window, 1–10 days
FIRMS_MAP_KEY=                 # server-side only (no VITE_ prefix); read by vite.config.js, never bundled
...
└── src/
    ...
    │   └── tracker.js    # blue 3D arrow pointing at the other pilot + distance readout
    ├── spacetimedb/
    │   └── client.js     # Phase 2: connect, subscribe, join_game, push/read-back
    ├── module_bindings/  # generated by `spacetime generate` (do not edit)
    ├── worlds/
    │   └── registry.js   # world configs (spawn, limits, boundary, layers, JARVIS) + ?world= resolution
    └── environmental/    # source → normalize → analyse → render → evidence → mission
        ├── providers.js / index.js   # FIRMS demo (bundled) or live (dev proxy), same output shape
        ├── firms.js / observations.js
        ├── priority.js / briefing.js # OBSERVATION PRIORITY rules, deterministic JARVIS
        ├── encoding.js / wildfireLayer.js / missionMarker.js   # Cesium
        ├── evidencePanel.js / mission.js
        └── data/         # firms-demo-california.csv + .meta.json (real, unmodified)

docs/
└── environmental-intelligence.md   # architecture, test procedure, limitations
