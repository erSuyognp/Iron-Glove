...
// src/module_bindings/hit_boss_reducer.ts
var hit_boss_reducer_default = {
  amount: t.u8()
};

// src/module_bindings/join_reducer.ts
var join_reducer_default = {
  name: t.string()
};

// src/module_bindings/update_pose_reducer.ts
var update_pose_reducer_default = {
  x: t.f32(),
  y: t.f32(),
  z: t.f32(),
  yaw: t.f32(),
...
// src/module_bindings/boss_table.ts
var boss_table_default = t.row({
  id: t.u8().primaryKey(),
  x: t.f32(),
  y: t.f32(),
  z: t.f32(),
...
// src/module_bindings/pilot_table.ts
var pilot_table_default = t.row({
...
var reducersSchema = reducers(
  reducerSchema("hit_boss", hit_boss_reducer_default),
  reducerSchema("join", join_reducer_default),
  reducerSchema("update_pose", update_pose_reducer_default)
);
...
function createIronGloveStdb(opts) {
  const host = toWs(opts?.host ?? "wss://maincloud.spacetimedb.com");
  const dbName = opts?.dbName ?? "iron-glove-suyog";
  let conn = null;
  let myId = null;
  const pilots = /* @__PURE__ */ new Map();
  let bossHp = 100;
  const bossListeners = /* @__PURE__ */ new Set();
  const pilotListeners = /* @__PURE__ */ new Set();
  const statusListeners = /* @__PURE__ */ new Set();
  const emitStatus = (msg) => statusListeners.forEach((cb) => cb(msg));
  ...
  const handle = {
    ...
    connect(name = "Pilot") {
      ...
      DbConnection.builder().withUri(host).withDatabaseName(dbName).onConnect((c, identity) => {
        conn = c;
        myId = identity.toHexString();
        emitStatus("online");
        c.db.boss.onInsert((_ctx, row) => {
          bossHp = row.hp;
          emitBoss();
        });
        c.db.boss.onUpdate((_ctx, _o, row) => {
          bossHp = row.hp;
          emitBoss();
        });
        c.db.pilot.onInsert((_ctx, row) => {
          const s = snapPilot(row);
          pilots.set(s.id, s);
          emitPilots();
        });
        c.db.pilot.onUpdate((_ctx, _o, row) => {
          const s = snapPilot(row);
          pilots.set(s.id, s);
          emitPilots();
        });
        c.db.pilot.onDelete((_ctx, row) => {
